  ...
  // post
    difference() {
      ...
      translate([ball_bearing_height / 2 , 0, flip * spring_z + 7])
        ...
        cylinder(h = post_height + ball_bearing_height + 10, d = ball_bearing_inner_diameter - .05);
   ...
   difference() {
      // outer shell
      translate([ball_bearing_height / 2 , 0, 2 * ball_bearing_outer_diameter + 9])
      sphere(r = ball_bearing_outer_diameter);
        
      translate([ball_bearing_height / 2 , 0, 2 * ball_bearing_outer_diameter + dish_offset + post_height])
       cube(2 * ball_bearing_outer_diameter, center = true);

      translate([ball_bearing_height / 2 , 0, 2 * ball_bearing_outer_diameter + 9])
      sphere(r = ball_bearing_outer_diameter - 4); 
     }
 }
tire_apparatus();
