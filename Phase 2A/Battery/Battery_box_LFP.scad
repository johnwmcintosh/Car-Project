...
use <../i2c.scad>
use <../Front_End/targus4mount.scad>
use <tobsun.scad>

module battery_box(show_battery_box = true, show_i2c = false, show_tray = false, show_tobsun = false) {
    ...
    if (show_battery_box) {
        translate([0, 0, battery_box_peg_h]) {
          ...
          translate([0 , lfp_main_box_y / 2 - tobsun_shelf_width - 30.7 , lfp_main_box_z + 5])
          // usb hub mount
          targus4mount(); 
        ...
        // i2c holder
         ...
         difference() {
           ...
           translate([2.9, 18.5, lfp_main_box_z + 40.6])
              rotate([-125, 0, 0])
                cylinder(h = 13, d = i2c_screw_hole_d);    
           
           // lower left screw hole
           translate([2.9, 4, lfp_main_box_z + 22])
              rotate([-125, 0, 0])
                cylinder(h = 20, d = i2c_screw_hole_d);      
            
           // cutout for the i2c solder points
           translate([-3, 15.5, lfp_main_box_z + 29.1])
           rotate([0, 90, 0])
            cylinder(h = 10, d = 4);
          }
         ...
         // i2c holder
        translate([-lfp_main_box_x / 2 + i2c_w - 3.4, -lfp_main_box_y / 2 - 4.3, wall_thickness - 2])
         difference() {
            // mount pre-cutout
             translate([-1,  0, lfp_main_box_z + 7.5])
             cube([7, 30, 35]);
           ...
           // cutout for the i2c solder points
           translate([-2, 15.5, lfp_main_box_z + 29.1])
           rotate([0, 90, 0])
            cylinder(h = 10, d = 4);
         }
        ...
        translate([lfp_main_box_x/2 - 6, -lfp_main_box_y / 2 - 4.3, wall_thickness - 2])
         difference() {
           ...
           translate([2.9, 4, lfp_main_box_z + 22])
            ...
            // cutout for the i2c solder points
           translate([-2.5, 15.5, lfp_main_box_z + 29.1])
              rotate([0, 90, 0])
                cylinder(h = 10, d = 4);
         }

       // i2c holder
          translate([lfp_main_box_x / 2 - i2c_w - 1.6, -lfp_main_box_y / 2 - 4.3, wall_thickness - 2]) 
         difference() {
            // mount pre-cutout
             translate([-1,  0, lfp_main_box_z + 7.5])
             cube([7, 30, 35]);
