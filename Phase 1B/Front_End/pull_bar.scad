...
module pull_bar(rotateit = false, bar_length_offset = 0) {
    rotation = rotateit ? 180 : 0;
    ...
    rotate([0,rotation,0])
    {
        ...
        difference() {
           ...
           if (rotation == 0 ){
              translate([pull_bar_length + bar_length_offset + 2 * pull_bar_peg_r - 2.2,  -pull_bar_peg_r - 1.8, -0.5])
              cube([1.9, 2.8, 1]);
           }
          else
          {
